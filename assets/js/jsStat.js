var jsStats = {
    ...
    variance: function (arr) {
        ...
        var sumOfSquares = jsStats.sumOfSquares(arr);
    ...
    standardDeviation: function (arr) {
        ...
        return Math.sqrt(jsStats.variance(arr));
    },
    equalIntervalBreaks: function (arr, numBreaks) {
        var min = jsStats.min(arr);
        var max = jsStats.max(arr);
        var median = jsStats.median(arr);
        var span = max - min;
        ...
        for (i in breaks) {
            breaks[i].numbers = jsStats.sort(breaks[i].numbers);
        }
        return breaks;
    },
    goodnessOfFit: function (arrPopulation, arrSample) {
        if (!jsStatsHelpers.isArray(arr)) {
            return false;
        }
        var ssp = jsStats.sumOfSquares(arrPopulation);
